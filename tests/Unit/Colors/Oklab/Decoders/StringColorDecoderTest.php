...
final class StringColorDecoderTest extends BaseTestCase
{
    ...
    public function testSupportsOklabString(): void
    {
        $decoder = new StringColorDecoder();
        $this->assertTrue($decoder->supports('oklab(0.5 0.1 -0.1)'));
        $this->assertTrue($decoder->supports('OKLAB(0.5 0.1 -0.1)'));
        $this->assertTrue($decoder->supports('oklab(0.5, 0.1, -0.1)'));
    }
    ...
    public function testDecodeInvalid(): void
    {
        ...
        $decoder->decode('oklab(invalid)');
    }

    public function testDecodeWithPercentageValues(): void
    {
        $decoder = new StringColorDecoder();
        $result = $decoder->decode('oklab(50% 10% -10%)');
    ...
    {
        $decoder = new StringColorDecoder();
        $result = $decoder->decode('oklab(0.5 0.1 -0.1 / 0.5)');
        $channels = $result->channels();
    ...
    {
        $decoder = new StringColorDecoder();
        $result = $decoder->decode('oklab(0.5 0.1 -0.1 / 50%)');
        $channels = $result->channels();
        $this->assertCount(4, $channels);
